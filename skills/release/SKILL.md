...
name: release
description: >-
  Repository-scoped release instructions for Tabora. Use this when the user asks
  to cut a release, create or push a tag, watch the Release workflow, run a
  validation build, or verify the Sparkle appcast for this repository. Releases
  are tag-driven, so do not create a version bump commit just to publish a
  release.
allowed-tools:
  - Bash
  - Read
  ...
  - Glob
metadata:
  short-description: Tag-driven release workflow for Tabora
---
...
- `Tabora.zip` is attached to the GitHub Release
- the matching version is present in `gh-pages/appcast.xml`
- the dispatch to `mkusaka/homebrew-tap` succeeded inside the release workflow
...
```bash
gh workflow run Release --field version=0.0.4
gh run list --workflow Release --limit 5
gh run watch
```
